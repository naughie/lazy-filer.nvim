...
You need to run `cargo build --release` before spawning a filer process. This is done by
```
build = function(plugin)
    require('lazy-filer').build_and_spawn_filer(plugin.dir)
end,
...
If you have any trouble, execute this function by yourself, or run
```
$ cargo build --release
```
directly in the plugin's root directory.


### Autocmd

Lazy-filer sets the autocmd `VimEnter` on the `setup()` to spawn a filer process. It fails if you have not `cargo build --release` yet.
If it is the case call `require('lazy-filer').fn.spawn_filer()` manually after building.
